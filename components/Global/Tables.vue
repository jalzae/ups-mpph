  ...
  <div>
    <table :id="id" class="text-left w-full border-collapse">
      <thead class="thead-light">
        <tr>
          <th
            class="py-4 px-6 bg-gray-800 font-bold uppercase text-sm text-white border-b border-gray-200"
          >
            #
          </th>
          <th
            class="py-4 px-6 bg-gray-800 font-bold uppercase text-sm text-white border-b border-gray-200"
            v-for="(item, int) in format.header"
            :key="int"
          >
            {{ item }}
          </th>
      ...
      <tbody v-else>
        ...
        >
          ...
          <td
            class="py-4 px-6 border-b border-gray-200"
            v-for="items in format.btn"
            :key="items.action"
          >
            <button
              v-if="item[items.model] == items.false"
              style="margin: 10px !important"
              :class="items.class"
              @click="action(items.action, item[items.key])"
              class="btn"
            >
            ...
            <button
              v-else
              style="margin: 10px !important"
              :class="items.classfalse"
              @click="action(items.actionfalse, item[items.key])"
              class="btn"
            >
          ...
          >
            <button
              ...
              style="margin: 10px !important"
              :class="items.class"
              @click="action(items.action, item[items.key], item[items.key2])"
              class="btn"
            >
          ...
          <td
            class="py-4 px-6 border-b border-gray-200"
            v-for="items in format.images"
            :key="items.action"
          >
            <img
              :src="item[items.src]"
              :alt="item[items.src]"
              @click="action(items.action, items[item.model])"
              style="max-width: 70px !important"
            />
          </td>
          <td
            v-if="format.action.length > 0"
            class="py-4 px-6 border-b border-gray-200"
          >
            <button
              ...
              style="margin: 10px !important"
              :class="act.class"
              @click="action(act.action, item[act.model])"
              class="btn"
            >
